...
const Costumer = () => {const customStyles = {
  ...
  useEffect(() => {
    const fetchData= async () => {
      try {
        ...
        console.log(token)
        const response = await axios.get(`http://localhost:8080/api/customer?pageNumber=${currentPage}`, {
          headers: {
            'X-API-TOKEN': token
          }
        });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if(editedRow) {
        await axios.put(`http://localhost:8080/api/customer/${editedRow.id}`, formDataToSend, {
          headers: {
            'X-API-TOKEN': token
          }
        })
      }else{
        const response = await axios.post('http://localhost:8080/api/customer?pageNumber=0', formDataToSend, {
          headers: {
            'X-API-TOKEN': token
          }
        });
    ...
    const handleConfirmDelete = async () => {
      const token = localStorage.getItem("AuthToken");
      try {
        await axios.delete(`http://localhost:8080/api/customer/${editedRow.id}`, {
          headers: {
            'X-API-TOKEN': token
          }
        });
